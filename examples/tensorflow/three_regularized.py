...
data_root = (
    "/Users/sam/srpub/examples/tensorflow/data/"  # Change me to store data elsewhere
)
...
with graph.as_default():
    ...
    weights2 = tf.Variable(tf.truncated_normal([NUM_HIDDEN_ONE, NUM_HIDDEN_TWO]))
    biases2 = tf.Variable(tf.zeros([NUM_HIDDEN_TWO]))

    weights3 = tf.Variable(tf.truncated_normal([NUM_HIDDEN_TWO, num_labels]))
    ...
    learning_rate = tf.train.exponential_decay(
        INITIAL_LEARNING_RATE, global_step, 1000, 0.9
    )
...
with tf.Session(graph=graph) as session:
    ...
    print(
        "Test accuracy: %.1f%%"
        % accuracy(test_prediction.eval(feed_dict=feed_dict), test_labels)
    )
