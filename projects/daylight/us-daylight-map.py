...
def alt_is_daytime(lat, lng):
    ...
    """
    # Constants
    zenith = math.radians(90.833)  # official zenith for sunrise/sunset (includes refraction)
    ...
    cos_omega = (math.cos(zenith) - math.sin(lat_rad) * math.sin(decl)) / (math.cos(lat_rad) * math.cos(decl))
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Display an ASCII map of the US with the current daylight terminator.")
