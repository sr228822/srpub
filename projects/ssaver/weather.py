    ...
    try:
        ...
        if "current" in weather_data:
            ...
            for field_name, field_value in fields:
                padded_name = field_name + "&nbsp;" * (max_field_length - len(field_name))
                output.append(f"{padded_name}: {field_value}")
        else:
            output.append(f"Location: {location}")
            output.append(f"Weather data unavailable - API error: {json.dumps(weather_data.get('reason', 'Unknown error'))}")
